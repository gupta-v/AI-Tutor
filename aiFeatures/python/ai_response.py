...
def create_primary_prompt_with_history(session: ChatSession):
    ...
    return ChatPromptTemplate.from_messages([
        ("system", "You are an AI Assistant that generates educational content based on retrieved information. "
                 "Your role is to analyze the data retrieved from the knowledge database and create "
                 "informative, well-structured topic explanations that address the user's query. "
...
def generate_response_without_retrieval(session_id: str, prompt: str, session_manager: ChatSessionManager):
    ...
    try:
        ...
        response = (prompt_template | llm_primary | StrOutputParser()).invoke({"query": prompt})
    ...
    while True:
        ...
        if use_retrieval:
            ...
            response = generate_response_without_retrieval(
                test_session_id, user_input, session_manager
            )
